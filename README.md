# 🚀 Jornada do Dev - Do estágio ao CEO de uma empresa especializada em Cryptomoedas
Projeto da disciplina de Programação Orientada a Objetos.
Jogo que simula a evolução de um profissional de TI, desde os primeiros passos como estagiário, até alcançar o topo da carreira como CEO de uma grande empresa.

<img width="500" height="500" alt="415c5459-594b-4e5a-a8c7-56671dfd3cb1-removebg-preview" src="https://github.com/user-attachments/assets/70e8f962-dc11-4e90-9a15-7dc744b93d79" />

## 📌 Descrição:
O jogo é uma simulação da carreira de um profissional de TI, começando pequeno, como estagiário, até se tornar um grande CEO.
O Jogo inclui:
1. ...
2. ...
3. ...

Tudo operando via menu em terminal, ...

## ❓ Features/Recursos do jogo:
1. Sistema de dinheiro (em token fictício "CoffeeCoins") e sistema de XP, ambos adquiridos ao trabalhar!
2. Lojinha! Gaste suas CoffeeCoins aqui, existem os cursos os acessórios personalizáveis!
3. Os cursos poder aumentar seu nível de XP, te tornando mais apto a subir de cargo, ou então podem liberar novas formas de trabalho!
4. Acessórios personalizáves para tornar sua experiência no jogo única, deixando tudo do seu jeito!
5. ...


## 💻 Programa em execução:


## 🗺️ Instruções de execução:


## 📨 Diagrama UML:


## 📦 Pacotes do código:

## 👤 Integrantes:
- Heitor Farias - [LinkedIn](https://www.linkedin.com/in/heitorfariassantos/)
- Nina Lira
- Matheus Souto
